...
## 2026-09-22 任务管理与布局调整

- 顶部“全选任务”改为“管理任务”：默认隐藏勾选框，管理模式下显示；列表勾选框放右侧，卡片保留右下角。完成管理退出，取消选择只清空勾选。
- 管理模式底部操作栏在零选择时仍显示。增强/生成一组，右侧全选/取消选择一组；无选择时禁用生成。Ctrl+A 自动进入管理并全选，文本编辑中的原生快捷键保留。
- 桌面预览栏保留固定 310px 网格列，取消中等窗口宽度缩为 260px 的规则；手机仍使用既有单列布局。
- 首页文件夹网格整体居中，保留卡片尺寸与行内顺序。
- 只修改前端；生产后端、ComfyUI 与本次中文增强/末秒承接任务不重启、不变更。
- 验证：frontend verify 的类型检查、单元测试、构建通过；布局回归在 1366/940/780px 验证固定预览宽度、右侧勾选框、操作分组及文件夹居中。
- 顶部按钮挤压回归：原布局在窄桌面把按钮压缩约 12px。改为左右导航按内容占宽、中间标题弹性省略，导航按钮不参与收缩。1600/940/780px 长标题回归通过。
- 管理模式相关批量增强、视频选择、手机点击回归 16 项通过，独立布局回归 2 项通过。
- 任务工具栏常驻：任务区分为固定标题行与可滚动任务集合，列表/卡片共用；滚动不再隐藏视图切换和管理任务入口。保留单一任务滚动区域，预留顶部标签展示空间。新增滚动回归先复现标题移动500px，再修复；相关桌面/手机7项通过。
- 悬浮操作栏避让底部状态栏：统一使用状态栏高度变量，操作栏位于状态栏上方16px，桌面管理模式和手机单选/管理模式均适用；不遮挡设置、运行摘要及运行中心。加入实际边界回归。

## 2026-09-22 首条电影任务超时误报与结果找回
- 中文增强26/26成功。首条视频 job-f6f02af317d546c2b7d8cd924da09915 在应用600秒等待上限时报 SHOTMILL_BRIDGE_TIMEOUT；ComfyUI 原 prompt d61da210-d5d4-4b11-9436-ffcccd9a21ac 实际约1690秒后成功，未发生模型执行失败。
- 先核对 Bridge 原逻辑 Job 已完成并有输出；SQLite 在线备份至 .shotmill/backups/before-timeout-recovery.db，保存原错误记录，然后通过 GenerationService.execute(recover=True) 取回原输出，未重复生成、未检查成片质量。
- 队列空闲时仅重启 ShotMill 后端，为本次进程设置 SHOTMILL_PROVIDER_TIMEOUT=7200，ComfyUI 未重启。该设置为运行环境覆盖，尚未修改默认600秒配置或实现超时后自动找回。
- .shotmill/runs/film-chinese-lastsecond/resume.py 从第二条继续，仍按前一条实际结果最后1秒承接；第二条 job-0b3b1b5b9a4346e39afafc90627737d4 已进入运行。
- 后续产品改进：将等待超时与远端执行失败区分，并允许按原逻辑Job恢复取回，避免把正常长耗时视频误报失败。
