...
test("mobile workspace top controls receive taps after task selection", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "全选任务", exact: true }).tap();
...
test("mobile controls remain tappable with a long selection scrolled out of view", async ({ page }) => {
  ...
  await openProject(page, project.title);
  await page.getByRole("button", { name: "全选任务", exact: true }).tap();
  await page.locator(".project-task-area").evaluate(el => { el.scrollTop = 300; });
  await page.getByRole("button", { name: "项目配置", exact: true }).tap();
  await expect(page.getByRole("dialog")).toBeVisible();
  await page.keyboard.press("Escape");
  await page.locator(".project-task-area").evaluate(el => { el.scrollTop = 0; });
  await page.getByRole("button", { name: "取消全选", exact: true }).tap();
  await expect(page.getByRole("button", { name: "全选任务", exact: true })).toBeVisible();
});
