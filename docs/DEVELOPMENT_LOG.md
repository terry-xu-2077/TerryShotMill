...
## 2026-09-22 用户提示词历史版本
- 用户标签新增历史下拉，与AI增强历史处于同一区域。版本显示序号和保存时间；选中旧版载入草稿，保存后生效，取消不改任务。
- Task新增独立user_prompt_history持久化字段，迁移0006；创建任务记录初始内容，只有用户提示词内容改变时追加记录。旧任务空历史在读取时展示当前内容，首次保存时留存原内容。恢复旧版会追加新版本，历史不覆盖。
- 历史由后端维护，不接受客户端提交的历史列表；遵循现有任务忙碌保护、revision并发检查、审核失效及任务隔离规则，不修改已提交Job快照。
- 后端verify、前端verify通过；浏览器验证选择历史、取消、保存恢复、刷新持久化与窗口尺寸稳定通过。
- 运行环境尚未部署：自动审批因连接中断拒绝进程检查，未重启生产后端、未迁移生产数据库。视频批量控制器仍运行。需要安全更新后端加载迁移0006后，现有项目才能使用完整历史功能。

### 用户历史改为手动存档（覆盖上一节自动记录规则）
- 新建任务、普通保存任务、恢复旧版均不追加历史；不再给旧任务自动补初始版本。
- 用户历史旁“保存版本”明确保存当前任务并存档当前用户提示词，只有该操作发送saveUserPromptVersion=true。后端生成版本ID和时间；前端不提交历史列表。
- 历史选择仅载入草稿，取消不生效。保留任务并发冲突与生成中只读保护。
- 生产后端尚未加载此改动，沿用前述部署阻塞说明。


## 2026-09-22 · 生成进度与主动停止

- 右侧生成进度区展示 ComfyUI 当前阶段、步数、阶段耗时、预计剩余与已执行阶段；任务列表/卡片底部增加无文字进度光条。估时从首步耗时开始，后续根据实际步速修正，明确仅估计当前阶段。
- 运行中任务使用“停止任务”按钮。确认弹窗关闭后立即显示旋转图标与“正在停止”，直到服务返回终态。停止按逻辑任务映射到精确 ComfyUI prompt_id，不使用全局中断。
- 主动中断持久化为 cancelled / USER_STOPPED，用户文案统一“用户停止”，不计为失败、不删除历史成片；停止时刚好完成则保留完成结果。排队移除仍为“已取消”。
- 回归覆盖确认取消不发请求、确认后弹窗立即关闭、禁用等待按钮、终态恢复生成按钮、用户停止与失败区分、旧结果保留、跨项目拒绝以及定向 ComfyUI 请求。
- backend/frontend/providers 三项 verify 均通过；进度与停止 Playwright 场景均通过。已备份生产数据库并迁移、重启应用后端，确认 26 个任务完整且没有运行/排队任务。没有重新生成视频。
- 之前受权限限制未加载的用户提示词手动历史功能也已随此次重启生效。

## 2026-09-22 · 单任务直接生成与预览收紧
- 单任务正常校验通过后直接提交，不弹确认；有风险、校验失败或提交异常才显示说明。批量与停止确认保持原行为。
- 右侧提示词预览从最多 8 行收为 4 行，保留字号和行高。
- 前端 verify 全通过；相关浏览器场景 9 项首轮通过，1 项修正提交后需重新选择的测试后通过。最后一轮测试结束清理临时数据库遇 Windows 文件占用，测试本身通过。

## 2026-09-22 · 提交生成后保留预览
- 根因：提交成功复用了取消选择逻辑，同时清空当前预览任务。改为只清除批量勾选，保留当前任务选择。
- 先通过浏览器回归复现提交后选中项消失，再验证修复后选中行与右侧任务信息持续显示；两个相关场景通过。

## 2026-09-22 · 长步骤进度不可回退为空态
- 原因：前端将超过 30 秒无事件视为无效，实际采样单步耗时数分钟，造成已显示的进度与估时反复消失。
- 删除按事件年龄清空进度的规则，持续显示最后一次有效步骤与估算，下一事件再更新。仅尚未收到数据时显示等待。
- 浏览器回归先复现 120 秒旧数据丢失，再验证列表、卡片与右栏均保留数据；测试通过，测试退出时临时数据库清理遇 Windows 文件占用。首步估时单元测试通过。

## 2026-09-22 · 运行中的柔和动效
- 进度填充内增加循环高光，保持实际进度长度不变；运行中任务条背景增加 3.6 秒主题色呼吸微光，透明度 2.5%–12%，不拦截点击。
- 浏览器验证动画名称、循环和进度数值通过；测试临时数据库清理仍偶发 Windows 文件占用。

## 2026-09-22 · 任务参数与提示词来源
- 任务列表/卡片移除缩略图时长，参数明确标注分辨率、时长、质量及用户/AI 提示词来源。
- Workspace 增加真实 promptSource 字段；兼容尚未重启的后端时从正式任务编辑接口读取来源，避免显示虚假的用户默认值。当前生成任务不重启、不受影响。

## 2026-09-22 · 片段承接结果自动解析
- 第五任务在前任务完成后仍阻断，因为缺失结果引用时只为尾帧承接自动解析上一任务。片段承接现在也在提交时解析上一任务结果，无需重新保存。保留跨项目与已失效引用校验。
- 先复现回归再修复，10 项上下文测试和 backend verify 通过。空闲后重启加载。

## 2026-09-22 · 独立下拉菜单风格
- 新增项目自有 Select，替换所有旧库 Select 和 PortalSelect 实现；胶囊触发按钮、24px 面板圆角、圆角选项、柔和选中背景，支持次级说明与图标。
- 默认撑满业务容器，修复视频历史右侧留空；保留定位、搜索、键盘导航、Escape、禁用和焦点恢复。
- 用户明确采用独立新风格，完成打磨后另行同步为 UI 库新风格类。此次未修改旧库发布版本。
